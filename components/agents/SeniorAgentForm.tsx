    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
                ...
                <div className="space-y-6">
                    <Card className="bg-zinc-900/40 border-zinc-800">
                        ...
                        <CardContent className="space-y-4">
                            <div><label className="text-xs font-bold text-zinc-500 uppercase mb-1 block">Nome do Agente</label><Input value={name} onChange={e => setName(e.target.value)} className="bg-zinc-950 border-zinc-800" placeholder="Ex: Roberto Diretor" autoFocus /></div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div><label className="text-xs font-bold text-zinc-500 uppercase mb-1 block">Tom de Voz</label><Input value={tone} onChange={e => setTone(e.target.value)} className="bg-zinc-950 border-zinc-800" placeholder="Ex: Estratégico" /></div>
                            </div>
        ...
        {step === 2 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-right-8">
                <div className="space-y-6">
                    <Card className="bg-zinc-900/40 border-zinc-800">
                        <CardHeader><CardTitle className="text-base text-zinc-100 flex items-center gap-2"><Bot className="w-4 h-4 text-purple-500" /> Prompt Mestre</CardTitle></CardHeader>
                        <CardContent className="space-y-4">
                            <div><div className="flex justify-between items-center mb-2"><label className="text-xs font-bold text-zinc-500 uppercase">Instrução Mestra</label><Button size="sm" variant="ghost" className="h-6 text-[10px] text-purple-400 hover:text-purple-300 hover:bg-purple-500/10 gap-1" onClick={() => setIsGeneratorOpen(true)}><Sparkles className="w-3 h-3" /> Mágica IA</Button></div><Textarea value={systemPrompt} onChange={e => setSystemPrompt(e.target.value)} className="bg-zinc-950 border-zinc-800 min-h-[300px] font-mono text-xs leading-relaxed" placeholder="Defina a inteligência superior do agente..." /></div>
                            <div><label className="text-xs font-bold text-zinc-500 uppercase mb-2 block flex items-center gap-2"><Target className="w-3 h-3 text-blue-500" /> Metodologia</label><select value={salesTechnique} onChange={e => setSalesTechnique(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-md p-2 text-sm text-zinc-200 outline-none">{SALES_TECHNIQUES.map(t => (<option key={t.id} value={t.id}>{t.label}</option>))}</select></div>
                ...
                <div className="space-y-6">
                    <Card className="bg-zinc-900/40 border-zinc-800"><CardHeader><CardTitle className="text-base text-zinc-100 flex items-center gap-2"><FileText className="w-4 h-4 text-orange-500" /> Base Expandida</CardTitle></CardHeader><CardContent className="space-y-4"><div className="bg-orange-500/5 border border-orange-500/20 p-4 rounded-lg"><p className="text-xs text-orange-200/80 leading-relaxed mb-3">O Agente Sênior suporta até <strong>20 arquivos</strong>.</p><label className={cn("flex items-center justify-center w-full h-20 border-2 border-dashed rounded-lg cursor-pointer transition-colors", files.length >= 20 ? "border-zinc-800 opacity-50 cursor-not-allowed" : "border-zinc-700 hover:border-orange-500/50 hover:bg-zinc-900")}><div className="flex flex-col items-center gap-1 text-zinc-500">{uploadingFile ? <Loader2 className="w-5 h-5 animate-spin" /> : <Upload className="w-5 h-5" />}<span className="text-xs font-medium">Upload Arquivo</span></div><input type="file" className="hidden" accept=".txt,.md,.docx,.pdf" onChange={handleFileUpload} disabled={files.length >= 20 || uploadingFile} /></label></div><div className="space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar">{files.map((file, idx) => (<div key={idx} className="flex items-center justify-between p-2 bg-zinc-950 border border-zinc-800 rounded group"><span className="text-xs text-zinc-200 truncate">{file.name}</span><button onClick={() => setFiles(prev => prev.filter((_, i) => i !== idx))} className="text-zinc-600 hover:text-red-500 p-1 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={14} /></button></div>))}</div></CardContent></Card>
                    <Card className="bg-zinc-900/40 border-zinc-800"><CardHeader><CardTitle className="text-base text-zinc-100 flex items-center gap-2"><LinkIcon className="w-4 h-4 text-cyan-500" /> Links Estratégicos</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex gap-2"><Input value={newLinkTitle} onChange={e => setNewLinkTitle(e.target.value)} placeholder="Título" className="bg-zinc-950 border-zinc-800 text-xs h-8" /><Input value={newLinkUrl} onChange={e => setNewLinkUrl(e.target.value)} placeholder="URL" className="bg-zinc-950 border-zinc-800 text-xs h-8" /><Button size="sm" onClick={() => { if(newLinkTitle && newLinkUrl) { setLinks([...links, {title: newLinkTitle, url: newLinkUrl}]); setNewLinkTitle(''); setNewLinkUrl(''); }}} className="h-8 bg-cyan-600 hover:bg-cyan-500 px-2"><Plus className="w-4 h-4" /></Button></div><div className="space-y-1">{links.map((link, idx) => (<div key={idx} className="flex items-center justify-between p-2 bg-zinc-950 border border-zinc-800 rounded text-xs group"><div className="flex flex-col overflow-hidden"><span className="font-bold text-zinc-300">{link.title}</span><span className="text-zinc-500 truncate">{link.url}</span></div><button onClick={() => setLinks(prev => prev.filter((_, i) => i !== idx))} className="text-zinc-600 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></button></div>))}</div></CardContent></Card>
                    <Card className="bg-zinc-900/40 border-zinc-800 border-l-4 border-l-blue-500"><CardHeader><CardTitle className="text-base text-zinc-100 flex items-center gap-2"><Phone className="w-4 h-4 text-blue-500" /> Notificação de Transbordo</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex gap-2"><Input value={newReportPhone} onChange={e => setNewReportPhone(e.target.value)} placeholder="551199..." className="bg-zinc-950 border-zinc-800" /><Button size="sm" onClick={handleAddReportPhone} className="bg-blue-600 hover:bg-blue-500"><Plus className="w-4 h-4" /></Button></div><div className="space-y-2">{reportPhones.map((phone, idx) => (<div key={idx} className="flex items-center justify-between p-2 bg-zinc-950 rounded border border-zinc-800 text-xs"><span className="text-zinc-300">{phone}</span><button onClick={() => setReportPhones(prev => prev.filter((_, i) => i !== idx))} className="text-zinc-500 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button></div>))}</div></CardContent></Card>
                    <div className="flex justify-between pt-4 border-t border-zinc-800 mt-4"><Button variant="ghost" onClick={() => setStep(1)} className="text-zinc-400 hover:text-white"><ArrowLeft className="w-4 h-4 mr-2" /> Voltar</Button><div className="flex gap-2"><Button onClick={() => setIsSimulatorOpen(true)} variant="outline" className="border-purple-500/50 text-purple-400 hover:bg-purple-500/10 hover:text-purple-300"><PlayCircle className="w-4 h-4 mr-2" /> Testar</Button><Button onClick={handleSave} disabled={loading} className="bg-purple-600 hover:bg-purple-500 text-white font-bold shadow-lg shadow-purple-500/20">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4 mr-2" />} Finalizar Sênior</Button></div></div>
                </div>
            </div>
        )}
        ...
        {step === 3 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-right-8">
                ...
                <div className="space-y-6">
                    <Card className="bg-zinc-900/40 border-zinc-800">
                        <CardHeader><CardTitle className="text-base text-zinc-100 flex items-center gap-2"><Settings className="w-4 h-4 text-pink-500" /> Ferramentas Ativas</CardTitle></CardHeader>
                        <CardContent className="space-y-6">
                            {/* GOOGLE DRIVE TOOL */}
                            <div className="p-4 bg-zinc-950/50 rounded-lg border border-zinc-800">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2"><Cloud className="w-5 h-5 text-blue-400" /><span className="text-sm font-bold text-zinc-200">Google Drive (Envio de Arquivos)</span></div>
                                    <div onClick={() => setUseDrive(!useDrive)} className={cn("w-10 h-5 rounded-full relative cursor-pointer transition-colors", useDrive ? "bg-green-600" : "bg-zinc-700")}><div className={cn("absolute top-1 w-3 h-3 bg-white rounded-full transition-all", useDrive ? "left-6" : "left-1")} /></div>
                                </div>
                                {useDrive && (
                                    <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                                        ...
                                        {loadingFolders ? (<div className="flex items-center gap-2 text-xs text-zinc-500"><Loader2 className="w-4 h-4 animate-spin" /> Carregando pastas...</div>) : (
                                            <div className="flex gap-2">
                                                <select value={driveFolderId} onChange={(e) => setDriveFolderId(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded p-2 text-xs text-white"><option value="">Selecione uma pasta...</option>{folders.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}</select>
                                                <Button size="sm" onClick={handleSyncFolder} disabled={syncingFolder || !driveFolderId} variant="outline" className="border-zinc-700 hover:bg-zinc-800">{syncingFolder ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}</Button>
                                            </div>
                                        )}
                                    </div>
                                )}
                            ...
                            {/* CALENDAR TOOL */}
                            <div className="p-4 bg-zinc-950/50 rounded-lg border border-zinc-800 flex items-center justify-between"><div className="flex items-center gap-2"><Calendar className="w-5 h-5 text-purple-400" /><span className="text-sm font-bold text-zinc-200">Agendamento (Calendar)</span></div><div onClick={() => setUseCalendar(!useCalendar)} className={cn("w-10 h-5 rounded-full relative cursor-pointer transition-colors", useCalendar ? "bg-green-600" : "bg-zinc-700")}><div className={cn("absolute top-1 w-3 h-3 bg-white rounded-full transition-all", useCalendar ? "left-6" : "left-1")} /></div></div>
                            {/* CRM TOOL */}
                            <div className="p-4 bg-zinc-950/50 rounded-lg border border-zinc-800 flex items-center justify-between"><div className="flex items-center gap-2"><Database className="w-5 h-5 text-yellow-400" /><span className="text-sm font-bold text-zinc-200">Consulta ao CRM</span></div><div onClick={() => setUseCRM(!useCRM)} className={cn("w-10 h-5 rounded-full relative cursor-pointer transition-colors", useCRM ? "bg-green-600" : "bg-zinc-700")}><div className={cn("absolute top-1 w-3 h-3 bg-white rounded-full transition-all", useCRM ? "left-6" : "left-1")} /></div></div>
                        </CardContent>
                    </Card>
                </div>
                <div className="space-y-6">
                    <Card className="bg-zinc-900/40 border-zinc-800 border-l-4 border-l-blue-500"><CardHeader><CardTitle className="text-base text-zinc-100 flex items-center gap-2"><Phone className="w-4 h-4 text-blue-500" /> Transbordo & Relatórios</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-xs text-zinc-400">Quando a IA transferir para um humano, enviar relatório para estes números:</p><div className="flex gap-2"><Input value={newReportPhone} onChange={e => setNewReportPhone(e.target.value)} placeholder="551199..." className="bg-zinc-950 border-zinc-800" /><Button size="sm" onClick={handleAddReportPhone} className="bg-blue-600 hover:bg-blue-500"><Plus className="w-4 h-4" /></Button></div><div className="space-y-2">{reportPhones.map((phone, idx) => (<div key={idx} className="flex items-center justify-between p-2 bg-zinc-950 rounded border border-zinc-800 text-xs"><span className="text-zinc-300">{phone}</span><button onClick={() => setReportPhones(prev => prev.filter((_, i) => i !== idx))} className="text-zinc-500 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button></div>))}</div></CardContent></Card>
                    <div className="flex justify-between pt-4 border-t border-zinc-800 mt-4"><Button variant="ghost" onClick={() => setStep(2)} className="text-zinc-400 hover:text-white"><ArrowLeft className="w-4 h-4 mr-2" /> Voltar</Button><div className="flex gap-2"><Button onClick={() => setIsSimulatorOpen(true)} variant="outline" className="border-purple-500/50 text-purple-400 hover:bg-purple-500/10 hover:text-purple-300"><PlayCircle className="w-4 h-4 mr-2" /> Testar</Button><Button onClick={handleSave} disabled={loading} className="bg-purple-600 hover:bg-purple-500 text-white font-bold shadow-lg shadow-purple-500/20">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4 mr-2" />} Finalizar Sênior</Button></div></div>
                </div>
            </div>
        )}
